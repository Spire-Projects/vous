...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <_Content
      ...
      position={position}
      className={cn(
        "z-50 min-w-[8rem] bg-white border border-white/60 rounded-2xl shadow-xl shadow-black/5 backdrop-blur-xl p-1",
        className,
      )}
      {...props}
    >
      <_ScrollUp className="flex items-center justify-center py-1">
        <ChevronUp size={12} className="text-vous-text-secondary" />
      </_ScrollUp>
      <SelectPrimitive.Viewport>{children}</SelectPrimitive.Viewport>
      <_ScrollDown className="flex items-center justify-center py-1">
        <ChevronDown size={12} className="text-vous-text-secondary" />
      </_ScrollDown>
    </_Content>
  </SelectPrimitive.Portal>
));
...
>(({ className, ...props }, ref) => (
  <_Label
    ref={ref}
    className={cn(
      "px-3 py-1.5 font-nav text-[10px] uppercase tracking-[0.15em] text-vous-text-secondary",
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <_ItemIndicator>
        <Check size={12} className="text-vous-gold" />
      </_ItemIndicator>
    </span>
