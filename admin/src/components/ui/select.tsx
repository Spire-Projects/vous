...
type DivRef = React.ForwardRefExoticComponent<
  React.PropsWithChildren<React.HTMLAttributes<HTMLDivElement>> &
    React.RefAttributes<HTMLDivElement> & {
      position?: "item-aligned" | "popper"; sideOffset?: number;
...
const SelectTrigger = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean }>(
  ({ className, children, ...props }, ref) => (
    <_Trigger
      ref={ref}
      className={cn(
        "flex h-9 w-full items-center justify-between border border-vous-border bg-white/90 px-4 py-2 text-sm font-sans text-vous-text rounded-2xl shadow-inner focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 disabled:opacity-50 transition-all",
        className
      )}
      {...props}
    >
      {children}
      <ChevronDown size={14} className="text-vous-text-secondary shrink-0 ml-2" />
    </_Trigger>
  )
);
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <_Content
      ref={ref}
      position={position}
      className={cn("z-50 min-w-[8rem] bg-white border border-white/60 rounded-2xl shadow-xl shadow-black/5 backdrop-blur-xl p-1", className)}
      {...props}
    >
      <_ScrollUp className="flex items-center justify-center py-1"><ChevronUp size={12} className="text-vous-text-secondary" /></_ScrollUp>
      <SelectPrimitive.Viewport>{children}</SelectPrimitive.Viewport>
      <_ScrollDown className="flex items-center justify-center py-1"><ChevronDown size={12} className="text-vous-text-secondary" /></_ScrollDown>
    </_Content>
  </SelectPrimitive.Portal>
));
...
const SelectLabel = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <_Label ref={ref} className={cn("px-3 py-1.5 font-nav text-[10px] uppercase tracking-[0.15em] text-vous-text-secondary", className)} {...props} />
  )
);
...
const SelectItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { value: string; disabled?: boolean }>(
  ({ className, children, ...props }, ref) => (
    <_Item
      ref={ref}
      className={cn(
        "relative flex cursor-default select-none items-center py-2 pl-8 pr-3 font-sans text-sm text-vous-text rounded-xl outline-none focus:bg-amber-50 data-[disabled]:opacity-50",
        className
      )}
      {...props}
    >
      <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
        <_ItemIndicator><Check size={12} className="text-vous-gold" /></_ItemIndicator>
      </span>
