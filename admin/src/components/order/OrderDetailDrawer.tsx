  ...
  return (
    <div className="border border-vous-border p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-nav text-[11px] uppercase tracking-[0.15em] text-vous-text">
          {title}
        </h3>
      </div>
      {children}
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full md:max-w-[680px] bg-vous-surface flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4 px-6 py-4 border-b border-white/40 shrink-0">
          <div>
            <p className="font-nav text-[11px] uppercase tracking-[0.15em] text-vous-text-secondary">
              Pedido
            </p>
            <h2 className="font-serif text-xl font-medium text-vous-text mt-0.5">
              {order.orderNumber}
            </h2>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-6 flex flex-col gap-5">
              ...
              <Section title="Historial de cambios" icon={<Clock size={14} />}>
                <div className="space-y-3">
                  {[...order.statusHistory].reverse().map((entry, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 pb-3 border-b border-white/40 last:border-0 last:pb-0"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        {entry.notes && (
                          <p className="font-sans text-xs text-vous-text-secondary leading-relaxed">
                            {entry.notes}
                          </p>
                        )}
            ...
            <Section title="Cliente" icon={<User size={14} />}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 font-sans text-[13px]">
                <div>
                  ...
                  <p className="text-vous-text truncate">
                    {order.customer.name || "—"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-vous-text break-all">
              ...
              <Section title="Información de envío" icon={<MapPin size={14} />}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 font-sans text-[13px]">
                  <div>
                    ...
                    <p className="text-vous-text capitalize">
            ...
            <Section title="Transportista externo" icon={<Truck size={14} />}>
              <p className="font-sans text-[11px] text-vous-text-secondary mb-2">
                Referencia Yango u otro (opcional).
              </p>
              <p className="font-sans text-sm text-vous-text">
                {order.carrierRef || (
                  <span className="text-vous-text-secondary italic">
              ...
              <div className="flex flex-col gap-3">
                {order.items.map((item, i) => (
                  <div key={i} className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-sans text-[13px] font-medium text-vous-text truncate">
                        {item.productName}
                      </p>
                      ...
                      <p className="font-sans text-[11px] text-vous-text-secondary mt-0.5">
                        {item.quantity} × Bs.{" "}
                        {item.unitPrice.toLocaleString("es-BO")}
                      </p>
                    </div>
                    <p className="font-nav text-[13px] font-semibold text-vous-text shrink-0">
              ...
              <div className="mt-4 pt-4 border-t border-white/40 font-sans text-[13px] flex flex-col gap-1.5">
                ...
                {!!order.discountAmount && order.discountAmount > 0 && (
                  <div className="flex justify-between text-emerald-700">
                    ...
                    <span>
                      -Bs. {order.discountAmount.toLocaleString("es-BO")}
                    </span>
                  </div>
                )}
            ...
            <Section title="Comprobante de pago" icon={<FileText size={14} />}>
              <div className="flex items-center gap-3 mb-3">
                <Badge
                  variant={order.paymentMethod === "qr" ? "gold" : "outline"}
                >
              ...
              {order.paymentProof ? (
                <div className="flex flex-col gap-3">
                  ...
                  >
                    <img
                      ...
                      alt="Comprobante"
                      className="max-h-48 w-auto max-w-full border border-vous-border object-contain"
                    />
                  </a>
                  <div className="flex gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 font-nav text-[10px] uppercase tracking-wider text-vous-text-secondary hover:text-vous-text transition-colors"
                    >
                ...
                <p className="font-sans text-[13px] text-vous-text-secondary italic">
      ...
      {showCancelConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            className="absolute inset-0 bg-vous-black/60"
            onClick={() => {
              setShowCancelConfirm(false);
              setCancelNote("");
            }}
          ...
          <div className="relative bg-white/80 backdrop-blur-lg border border-white/60 rounded-3xl shadow-xl shadow-black/5 overflow-hidden p-6 max-w-sm w-full mx-4 shadow-xl">
            ...
            <p className="font-sans text-sm text-vous-text-secondary mb-4">
              Cancelarás <strong>{order.orderNumber}</strong> y se repondrá el
            ...
            <p className="font-sans text-xs text-vous-text-secondary mb-5 italic">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={saving || !cancelNote.trim()}
                className="flex-1"
              >
