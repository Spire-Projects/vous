...
export function BannersTab() {
  const { banners, loading, create, update, remove, toggleActive, reorder } =
    useBanners();
  ...
  const responsiveCount = banners.filter(
    (b) => b.mobileImageUrl || b.tabletImageUrl,
  ).length;
            ...
            {banners.map((banner, idx) => (
              ...
              >
                ...
                <div className="shrink-0 flex items-center gap-2">
                  <div className="relative">
                    <div className="w-24 h-14 overflow-hidden border border-vous-border bg-white">
                      {banner.imageUrl ? (
                        ...
                        <div className="w-full h-full bg-white/90 flex items-center justify-center text-vous-text-secondary text-[10px]">
                          Sin imagen
                        </div>
                      )}
                    </div>
                    <Monitor
                      size={10}
                      className="absolute -bottom-3 left-1/2 -translate-x-1/2 text-black/40"
                    />
                  </div>
                  <div className="relative">
                    <div
                      className={`w-12 h-14 overflow-hidden border ${banner.tabletImageUrl ? "border-black/40" : "border-vous-border border-dashed"} bg-white`}
                    >
                      {banner.tabletImageUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-black/30 text-[9px]">
                          —
                        </div>
                      )}
                    </div>
                  ...
                  <div className="relative">
                    <div
                      className={`w-7 h-14 overflow-hidden border ${banner.mobileImageUrl ? "border-black/40" : "border-vous-border border-dashed"} bg-white`}
                    >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-vous-text-muted font-nav mt-1">
                    Orden: {banner.order} · CTA: {banner.ctaText} →{" "}
                    {banner.ctaUrl}
                  </p>
                </div>
