...
import {
  ColorVariantPicker,
  type ColorItem,
} from "@/components/shared/ColorVariantPicker";
...
export function StepColors({
  ...
  return (
    <section className="space-y-4">
      ...
      <HelpText>
        Cada color puede tener sus propias fotos. Cuando el cliente seleccione
        un color, vera las fotos especificas. Las fotos generales se muestran
        cuando no hay color seleccionado.
      </HelpText>
      ...
      <div className="space-y-1 pt-2">
        ...
        <div className="flex flex-wrap gap-1.5 mb-2">
          {productImages.map((img, i) => (
            <div
              key={i}
              className="relative w-14 h-14 border border-vous-border"
            >
              <img src={img} alt="" className="w-full h-full object-cover" />
              <button
                type="button"
                onClick={() =>
                  onProductImagesChange(productImages.filter((_, j) => j !== i))
                }
        ...
        <ImagePicker
          value=""
          onChange={(url) => onProductImagesChange([...productImages, url])}
          folder="vous/products"
          label="Agregar foto general"
          aspect="square"
        />
      </div>
    </section>
...
export function StepVariants({
  ...
  return (
    <section className="space-y-3">
      ...
      {!hasAny ? (
        <HelpText>
          Aun no has agregado colores ni tallas. Ve a los pasos Colores y Tallas
          para definir las opciones.
        </HelpText>
      ) : (
        <HelpText>
          Agrega manualmente las combinaciones de color y talla que existen.
          Cada variante tiene su propio stock y SKU opcional.
        </HelpText>
      )}
      <VariantEditor
        colors={colors}
