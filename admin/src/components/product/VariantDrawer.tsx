...
export function VariantDrawer({ product, onClose }: VariantDrawerProps) {
  const { variants, loading, create, update, remove } = useVariants(
    product?.id ?? null,
  );
    ...
    <>
      <Dialog open={!!product} onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="max-w-2xl">
          ...
          {loading ? (
            <p className="text-center text-sm text-vous-text-secondary py-8">
              Cargando…
            </p>
          ) : variants.length === 0 ? (
            <p className="text-center text-sm text-vous-text-secondary py-8">
              ...
              <TableBody>
                {variants.map((v) => (
                  <TableRow key={v.id}>
                    ...
                    <TableCell className="text-vous-text-secondary text-xs">
                      {v.sku ?? "—"}
                    </TableCell>
                    <TableCell>
