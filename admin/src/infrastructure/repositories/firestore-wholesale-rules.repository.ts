...
function mapData(data: Record<string, unknown>): WholesaleRules {
  return {
    minimumPurchaseAmount:
      (data.minimumPurchaseAmount as number) ??
      DEFAULT_RULES.minimumPurchaseAmount,
    minimumPurchaseUnits:
      (data.minimumPurchaseUnits as number) ??
      DEFAULT_RULES.minimumPurchaseUnits,
    discountPercentage:
      (data.discountPercentage as number) ?? DEFAULT_RULES.discountPercentage,
    allowSizeSelection:
      (data.allowSizeSelection as boolean) ?? DEFAULT_RULES.allowSizeSelection,
