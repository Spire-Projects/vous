...
export function useSocialPosts() {
  ...
  const create = useCallback(
    async (input: CreateSocialPostInput) => {
  ...
  const update = useCallback(
    async (id: string, input: UpdateSocialPostInput) => {
  ...
  const reorder = useCallback(
    async (items: SocialPost[]) => {
      await Promise.all(
        items.map((p, idx) =>
          setSocialPostOrder(firestoreSocialPostRepository, p.id, idx),
        ),
      );
      await fetchPosts();
    },
