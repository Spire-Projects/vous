...
const PLACEHOLDER_BG = "bg-black/5";
      ...
      {loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-8">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="aspect-[3/4] animate-pulse bg-black/10" />
          ))}
        </div>
      )}
      ...
      {!loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-8">
          ...
          {lookProducts.length === 0 && (
            <p className="col-span-full text-center font-sans text-sm text-black/50 py-12">
...
function ProductLookCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/producto/${product.slug}`} className="group cursor-pointer">
      <div
        className={`relative aspect-[3/4] overflow-hidden mb-5 ${!imageUrl ? `${PLACEHOLDER_BG}` : "bg-black"}`}
      >
        {imageUrl ? (
          <img
            src={imageUrl}
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <span className="bg-white text-black px-6 py-2 font-nav text-[10px] font-semibold tracking-[0.2em] uppercase hover:bg-black hover:text-white transition-colors">
            VER
          </span>
        </div>
      </div>
