...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <_Close className="absolute right-4 top-4 text-black/50 hover:text-black transition-colors">
        <X size={16} />
...
const DialogTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <_Title ref={ref} className={cn("font-serif text-xl text-black", className)} {...props} />
  )
);
...
>(({ className, ...props }, ref) => (
  <_Description ref={ref} className={cn("font-sans text-sm text-black/50", className)} {...props} />
));
