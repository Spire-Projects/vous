...
const REQUESTS = [
  {
    ...
    statusStyle: "bg-black/10 text-black",
...
export default function MayoristasPage() {
  return (
    <div className="bg-white min-h-screen">
      {/* Hero */}
      <div className="bg-black py-20 md:py-28 px-5 md:px-20">
      ...
      <div className="max-w-[1440px] mx-auto px-5 md:px-20 py-16 md:py-20">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          ...
          <div className="lg:w-80 shrink-0 space-y-5">
            <h2 className="font-serif text-xl text-black">Estado de Solicitudes</h2>
            {REQUESTS.map(({ ref, status, statusStyle, desc }) => (
              <div key={ref} className="border border-black/10 p-5 space-y-2">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <p className="font-nav text-[10px] tracking-[0.15em] uppercase text-black/50">
                    Ref {ref}
                  </p>
                  <span
                ...
                <p className="font-sans text-xs text-black/50 leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
