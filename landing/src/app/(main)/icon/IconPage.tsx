...
export function IconPage() {
  return (
    <>
      {/* Hero */}
      <section className="relative bg-vous-soft-black min-h-[50vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#3d2e15] via-[#2a2015] to-[#0d0d0b]" />
        <div className="relative z-10 text-center px-5 max-w-3xl mx-auto">
          <span className="font-nav text-[11px] tracking-[0.3em] uppercase text-vous-gold mb-4 block">
            Inspo Outfits
          </span>
      ...
      {/* Influencers Grid */}
      <section className="py-16 md:py-28 bg-vous-warm-white">
        <div className="max-w-[1440px] mx-auto px-5 md:px-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {INFLUENCERS.map((inf) => (
              <div
                key={inf.id}
                className="group bg-white border border-vous-gray-light/50 rounded-2xl overflow-hidden hover:shadow-xl hover:border-vous-gold/40 transition-all duration-300"
              >
                {/* Photo */}
                <div className="aspect-[3/4] bg-vous-cream relative overflow-hidden">
                  {inf.imageUrl ? (
                    <img
                      src={inf.imageUrl}
                      alt={inf.name}
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-4">
                      <div className="w-24 h-24 rounded-full bg-vous-gold/10 flex items-center justify-center">
                        <span className="font-serif text-4xl text-vous-gold">{inf.name[0]}</span>
                      </div>
                      <div className="text-center">
                        <span className="font-nav text-[11px] uppercase tracking-[0.2em] text-vous-gray block">
                          {inf.name}
                        </span>
                        <span className="font-sans text-[10px] text-vous-gray/60 mt-1 block">
                          {inf.bio}
                        </span>
                      </div>
                    </div>
                ...
                <div className="p-6 space-y-4">
                  <div>
                    <h3 className="font-serif text-xl text-vous-soft-black">{inf.name}</h3>
                    <p className="font-sans text-[12px] text-vous-gray mt-1">{inf.bio}</p>
                  </div>

                  <div className="flex items-center gap-4 pt-2 border-t border-vous-gray-light/30">
                    {inf.instagramUrl && (
                      <a
                        href={inf.instagramUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-vous-text-secondary hover:text-vous-gold text-[11px] font-sans transition-colors"
                      >
                    ...
                    {inf.tiktokUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 text-vous-text-secondary hover:text-vous-accent-purple text-[11px] font-sans transition-colors"
                      >
      ...
      {/* CTA */}
      <section className="py-16 md:py-24 bg-vous-soft-black">
        <div className="max-w-[1440px] mx-auto px-5 md:px-20 text-center">
          <Star size={32} className="text-vous-gold mx-auto mb-4" />
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-vous-gold text-vous-soft-black px-6 py-3 rounded-xl font-nav text-[12px] uppercase tracking-wide hover:bg-vous-gold-light transition-colors"
          >
            Colaborar con VOUS
          </a>
        </div>
